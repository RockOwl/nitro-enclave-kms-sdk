...
func forward(in, out net.Conn) {
	...
	go func() {
		if _, err := io.Copy(in, out); err != nil && !strings.Contains(err.Error(), annoyingErr) {
			l.Printf("Error while forwarding to %s, err: %s \n", in.RemoteAddr(), err)
		}
		in.Close()
		out.Close()
		wg.Done()
		fmt.Println("proxy forward ===> in-->out done")
	}()
	go func() {
		...
		if _, err := io.Copy(out, in); err != nil && !strings.Contains(err.Error(), annoyingErr) {
			l.Printf("Error while forwarded to %s, err: %s \n", out.RemoteAddr(), err)
		}
		in.Close()
		out.Close()
		wg.Done()
		fmt.Println("proxy forward ===> out-->in done")
	}()
	wg.Wait()
